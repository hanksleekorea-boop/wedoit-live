...
import {installMaturityUI} from './happyscan-maturity-ui.mjs';
import {installCorrections} from './happyscan-corrections-ui.mjs';
import {installDrafts} from './happyscan-drafts-ui.mjs';
import {installExports} from './happyscan-exports-ui.mjs';
import {practiceFields,executionFromInput} from './happyscan-practice-input.mjs';
...
let store,drafts,records=[],dailyValue=null,answers={},question=0,toastTimer,pendingInstall,focusReturn,scanStarted=false,consent=false,diaryDraft='',stage1,stage2,stage3,contentLibrary,installContentLibrary,installStage1,installStage2,installStage3,methodsLoaded=false;
const hasDraft=()=>Boolean(drafts?.hasPending())||Boolean(corrections?.hasDraft())||Object.values(actionNotes).some(v=>v.trim())||Object.values(actionInputs).some(v=>v.minutes||v.fit&&v.fit!=='not-rated')||Object.keys(answers).length>0||dailyValue!==null||Boolean($('dailyNote').value.trim())||Boolean(diaryDraft.trim())||Boolean(stage2?.hasDraft())||Boolean(stage3?.hasDraft());
const actionNotes={},actionInputs={};
...
function ensureConsent(callback){
  ...
  modal('시작하기 전에 알아두세요',`<p>${DISCLAIMER}</p><p>응답은 이 브라우저에만 저장됩니다. 외부로 자동 전송하지 않아요. 원하지 않는 질문은 건너뛸 수 있습니다.</p><p>작성 중인 측정·회고·생활·실천 메모는 이 기기에 초안으로 보관합니다. ‘초안 이어쓰기·삭제’에서 지울 수 있으며 정식 기록·점수·JSON 백업과는 별개입니다. 공용 기기에서는 사용 후 초안과 기록을 확인해 주세요.</p><label class="consent"><input type="checkbox" id="understood">만 18세 이상이며, 자체 성찰 도구와 기기 저장 안내를 이해했습니다.</label><button class="btn primary" id="acceptNotice" disabled>이해했어요, 시작할게요</button><p class="small muted">미성년자 전용 도구와 보호 절차는 아직 제공하지 않습니다.</p>`);
...
async function route(){
  const [rawPage,query='']=location.hash.slice(1).split('?'),params=new URLSearchParams(query);let page=rawPage||'home';if(!['home','measure','scan','actions','report','me','about','help','library'].includes(page))page='home';
  ...
  if(!contentLibrary&&page==='library'){({installContentLibrary}=await import('./happyscan-library.mjs'));contentLibrary=installContentLibrary({modal,close,getRecords:()=>records});contentLibrary.render();}
  if(!stage2&&['actions','me','help','report'].includes(page)){({installStage2}=await import('./happyscan-stage2-ui.mjs'));stage2=installStage2({getRecords:()=>records,save,ensureConsent,modal,close,toast});stage2.render();}
  if(!stage3&&['measure','actions','report','me','help'].includes(page)){({installStage3}=await import('./happyscan-stage3-ui.mjs'));stage3=installStage3({getRecords:()=>records,save,ensureConsent,modal,close,toast,getDrafts:()=>drafts});stage3.render();}
  if(page==='actions')$('actionArea').value=DIMENSIONS.some(d=>d.id===params.get('area'))?params.get('area'):'all';
  if(document.documentElement.dataset.happyscanReady==='true'){
    if(page==='actions'){$('actionArea').value=DIMENSIONS.some(d=>d.id===params.get('area'))?params.get('area'):'all';renderActions();}
    if(page==='report')report();
    else {stage2?.render();stage3?.render();}
  }
  document.querySelectorAll('.view').forEach(v=>v.hidden=v.id!==`view-${page}`);
  document.querySelectorAll('[data-nav]').forEach(n=>{if(n.dataset.nav===(page==='scan'?'measure':page==='library'&&n.closest('.bottomnav')?'actions':['about','help'].includes(page)?'me':page))n.setAttribute('aria-current','page');else n.removeAttribute('aria-current');});
  ...
  if(page==='library')contentLibrary?.render();maturity?.render();
  if(page==='actions'&&params.get('program')){const id=params.get('program');const target=[...document.querySelectorAll('[data-program-start],[data-four-start]')].find(b=>(b.dataset.programStart||b.dataset.fourStart)===id);setTimeout(()=>target?.scrollIntoView({block:'center'}),0);}
...
function routeFailed(error){$('saveWarning').textContent='이 화면을 열지 못했어요. 연결을 확인하고 다시 시도해 주세요. 기존 기록은 보존되어 있습니다.';$('saveWarning').hidden=false;}
window.addEventListener('hashchange',()=>void route().catch(routeFailed));
window.addEventListener('offline',()=>void route().catch(routeFailed));
...
function overview(){
  ...
  $('homeActions').innerHTML=recommend(last?.answers,filteredActions(ACTIONS,records.find(r=>r.type==='preferences')??{})).map(actionCard).join('')||'<p>현재 조건에 맞는 제안이 없어요. 쉬어도 좋고 내 공간에서 조건을 바꿔도 좋아요.</p>';
...
function renderQuestion(){
  ...
  ratings($('scanRating'),answers[d.id],n=>{answers[d.id]=n;$('scanNext').disabled=false;persistScan();});
...
function start(kind){if(!store)return toast('안전한 저장을 준비하지 못했어요. 저장 안내를 확인해 주세요.');ensureConsent(()=>{
  if(drafts?.has(kind)&&!drafts.isActive(kind))return drafts.open();
  ...
  else{modal('하루 돌아보기','<p class="muted">어떤 상황에서 어떤 느낌이 있었나요? 해석하거나 긍정적으로 바꿀 필요는 없어요.</p><div class="field"><label for="diaryText">내 하루의 한 장면</label><textarea id="diaryText" maxlength="2000" rows="6" placeholder="내가 기억하고 싶은 순간을 적어보세요."></textarea></div><p class="small muted">입력은 이 기기에 초안으로 보관합니다. 화면 위의 초안 보관 상태를 확인하세요.</p><button class="btn primary" id="saveDiary">회고 저장하기</button><button class="btn" id="discardDiary">임시 글 비우기</button>');$('diaryText').value=diaryDraft;$('diaryText').oninput=e=>{diaryDraft=e.target.value;void drafts?.save('diary',{note:diaryDraft});};$('discardDiary').onclick=async()=>{await drafts?.remove('diary');diaryDraft='';$('diaryText').value='';};$('saveDiary').onclick=async()=>{const note=$('diaryText').value.trim();if(!note)return toast('남기고 싶은 내용을 입력해 주세요.');if(await save('diary',{note},$('saveDiary'))){await drafts?.remove('diary');diaryDraft='';close();}};}
});}
async function refresh(){records=await store.all();overview();maturity.render();const page=location.hash.slice(1).split('?')[0]||'home';if(page==='actions')renderActions();if(page==='report')report();else{stage2?.render();stage3?.render();}$('recordCount').textContent=`행복 기록 ${records.length}건 · 기존 활동 기록과 별도 보관`;$('dailySaved').textContent=rows('daily').some(r=>dayKey(r.createdAt)===dayKey(Date.now()))?'오늘의 기분 기록이 저장되어 있어요. 새 기록은 덮어쓰지 않고 추가됩니다.':'';}
...
function persistScan(){if(!Object.keys(answers).length)return;void drafts?.save('weekly',{answers:{...answers},question}).then(()=>$('draftStatus').textContent=`${Object.keys(answers).length}개 응답 · ${drafts.status()}`);}
function persistDaily(){if(dailyValue!==null||$('dailyNote').value)void drafts?.save('daily',{value:dailyValue,note:$('dailyNote').value});}
function daily(){ratings($('dailyRating'),dailyValue,n=>{dailyValue=n;$('saveDaily').disabled=!store;persistDaily();});$('saveDaily').disabled=dailyValue===null||!store;}
$('dailyNote').oninput=persistDaily;
$('saveDaily').onclick=()=>ensureConsent(async()=>{if(dailyValue===null)return;if(await save('daily',{value:dailyValue,note:$('dailyNote').value},$('saveDaily'))){await drafts?.remove('daily');dailyValue=null;$('dailyNote').value='';daily();}});
async function finish(){
  ...
  if(await save('scan',{answers:{...answers},instrument:'hs-eight-v1'},$('scanNext'))){await drafts?.remove('weekly');scanStarted=false;answers={};question=0;location.hash='report';modal('나만의 출발점을 기록했어요',`<p>이번 결과는 <b>${rows('scan')[0].score}/100</b>입니다. 좋고 나쁨을 판정하는 임상 기준점이 아니에요.</p><p>${DISCLAIMER}</p><p>8개 응답의 단순 평균 × 10입니다. 마음에 걸리는 단면에서 부담 없는 실천 하나를 골라보세요.</p><a href="#actions" class="btn primary" id="resultActions">나에게 맞는 실천 보기</a>`);$('resultActions').onclick=close;}
}
$('scanBack').onclick=()=>{if(question>0)question--;renderQuestion();persistScan();};
$('scanNext').onclick=()=>{if(question<7){question++;renderQuestion();persistScan();$('questionTitle').focus();}else finish();};
$('scanSkip').onclick=()=>{delete answers[DIMENSIONS[question].id];if(question<7){question++;renderQuestion();persistScan();}else finish();};
$('cancelScan').onclick=e=>{if(!Object.keys(answers).length){scanStarted=false;return;}e.preventDefault();modal('측정을 그만둘까요?','<p>아직 완료된 측정은 아닙니다. 초안으로 보관하거나 임시 응답을 지울 수 있어요.</p><button class="btn primary" id="keepScanDraft">초안 보관하고 돌아가기</button><button class="btn danger" id="confirmCancel">임시 응답 지우고 돌아가기</button>');$('keepScanDraft').onclick=async()=>{if(await drafts?.save('weekly',{answers:{...answers},question})){resetInputs('weekly');drafts.park('weekly');close();location.hash='measure';toast('초안 보관함에서 이어 쓸 수 있어요.');}};$('confirmCancel').onclick=async()=>{await drafts?.remove('weekly');answers={};question=0;scanStarted=false;close();location.hash='measure';};};
...
function renderActions(){
  const filter=$('actionArea').value,query=$('actionSearch')?.value.trim().toLowerCase()||'',allowed=filteredActions(ACTIONS,records.find(r=>r.type==='preferences')??{}),visible=allowed.filter(a=>(filter==='all'||a.area===filter)&&[a.title,a.steps,a.alternative].join(' ').toLowerCase().includes(query));$('actionLibrary').innerHTML=visible.map(actionCard).join('')||'<p class="empty">조건에 맞는 실천이 없어요. 검색어나 내 공간의 제외 조건을 바꿔보세요.</p>';if($('actionSearchStatus'))$('actionSearchStatus').textContent=`${visible.length}개 실천`;
  const chosen=latestActions().filter(r=>r.status==='planned');
  $('myActions').innerHTML=chosen.length?`<div class="grid-3">${chosen.map(r=>{const a=ACTIONS.find(a=>a.id===r.actionId);return `<article class="card"><span class="badge">내가 선택한 실천</span><h3 style="margin-top:12px">${a.title}</h3><p class="small muted">${a.steps}</p>${practiceFields(a,actionNotes[a.id]??'',actionInputs[a.id]??{})}<div class="card-actions"><button class="btn primary" data-action-status="done" data-id="${a.id}">실천했어요</button><button class="btn" data-action-status="skipped" data-id="${a.id}">이번엔 쉬기</button></div></article>`;}).join('')}</div>`:'<div class="empty">아래에서 나에게 맞는 실천 하나를 골라보세요.<br>고르는 것만으로 완료 기록이 생기지는 않아요.</div>';
}
function report(){
...
$('deleteAll').onclick=()=>{
  modal('행복 기록 전체 삭제','<p>이 브라우저의 해피스캔 행복 응답·기분·회고·실천·프로그램·생활 기록과 설정, 완료 전 초안을 삭제합니다. 이전 활동 도구 기록, 클라우드 자료, 내려받은 파일은 별개입니다. 이 작업은 되돌릴 수 없어요.</p><p>필요하다면 먼저 창을 닫고 JSON 백업을 받아 주세요. 초안은 백업에 포함되지 않으므로 필요한 내용은 별도로 보관하세요.</p><div class="field"><label for="deleteWord">확인하려면 ‘삭제’를 입력하세요</label><input id="deleteWord" autocomplete="off"></div><button class="btn danger" id="confirmDelete" disabled>이 브라우저의 행복 기록·초안 삭제</button>');
  $('deleteWord').oninput=e=>$('confirmDelete').disabled=e.target.value!=='삭제';
  $('confirmDelete').onclick=async()=>{if(!store)return;const b=$('confirmDelete');b.disabled=true;try{await drafts?.clear();await store.clear();await refresh();channel?.postMessage('cleared');close();toast('이 브라우저의 행복 기록과 초안을 삭제했어요.');}catch(e){toast(e.message);b.disabled=false;}};
};
...
$('printReport').onclick=()=>window.print();$('actionArea').onchange=()=>{location.hash='actions'+($('actionArea').value==='all'?'':'?area='+$('actionArea').value);renderActions();};
...
document.addEventListener('input',e=>{const id=e.target.dataset.actionNote||e.target.dataset.actionExecution;if(id){if(e.target.dataset.actionNote)actionNotes[id]=e.target.value;else actionInputs[id]={...(actionInputs[id]??{}),[e.target.dataset.executionField]:e.target.value};void drafts?.save('action:'+id,{note:actionNotes[id]??'',minutes:actionInputs[id]?.minutes??'',fit:actionInputs[id]?.fit??'not-rated'});}});
document.addEventListener('click',e=>{
  ...
  if(b.dataset.actionDetail){const a=ACTIONS.find(a=>a.id===b.dataset.actionDetail);if(!a)return;const last=rows('scan')[0];
    ...
    $('planAction').onclick=()=>ensureConsent(async()=>{if(!filteredActions(ACTIONS,records.find(r=>r.type==='preferences')??{}).some(x=>x.id===a.id))return toast('현재 내 실천 조건에서 제외된 방법이에요. 조건을 먼저 확인하거나 다른 실천을 골라주세요.');if(latestActions().some(r=>r.actionId===a.id&&r.status==='planned')){close();toast('이미 선택한 실천이에요.');return;}if(await save('action',{actionId:a.id,status:'planned'},$('planAction'))){close();location.hash='actions';}});
  }
  if(b.dataset.actionStatus)ensureConsent(async()=>{const id=b.dataset.id;let execution;try{execution=executionFromInput(id,$('minutes-'+id)?.value,$('fit-'+id)?.value);}catch(e){toast(e.message);$('minutes-'+id)?.focus();return;}if(await save('action',{actionId:id,status:b.dataset.actionStatus,note:$('note-'+id)?.value.trim()||'',execution},b)){await drafts?.remove('action:'+id);delete actionNotes[id];delete actionInputs[id];toast(b.dataset.actionStatus==='done'?'실천을 남겼어요. 기분 변화는 별도로 기록해 보세요.':'쉬기로 한 선택을 기록했어요. 벌점은 없어요.');}});
  if(b.dataset.remove){const id=b.dataset.remove;if(records.some(r=>r.baselineId===id||r.endScanId===id))return toast('프로그램이나 실험에서 참조하는 측정이에요. 먼저 백업한 뒤 관련 과정과 삭제 범위를 확인해 주세요.');modal('이 기록을 삭제할까요?','<p>선택한 기록 한 건만 삭제합니다. 삭제 후 되돌릴 수 없어요.</p><button class="btn danger" id="confirmRemove">이 기록 삭제</button>');$('confirmRemove').onclick=async()=>{if(!store)return;const b=$('confirmRemove');b.disabled=true;try{await store.remove(id);await refresh();channel?.postMessage('changed');close();toast('선택한 기록을 삭제했어요.');}catch(e){toast(e.message);b.disabled=false;}};}
});
...
if(channel)channel.onmessage=e=>{if(e.data==='cleared'){resetInputs();close();toast('다른 창에서 이 브라우저의 행복 기록과 초안을 삭제했어요.');}void drafts?.reload().catch(e=>toast(e.message));if(store)refresh().catch(e=>toast(e.message));};
...
const maturity=installMaturityUI({getRecords:()=>records,modal,close,download,toast,renderActions});
installExports({getRecords:()=>records,modal,close,download,toast});
const corrections=installCorrections({getRecords:()=>records,getStore:()=>store,modal,close,toast,onChanged:async()=>{await refresh();channel?.postMessage('changed');}});
function resetInputs(key){if(!key||key==='weekly'){answers={};question=0;scanStarted=false;}if(!key||key==='daily'){dailyValue=null;$('dailyNote').value='';daily();}if(!key||key==='diary')diaryDraft='';for(const map of [actionNotes,actionInputs])if(!key)Object.keys(map).forEach(id=>delete map[id]);else if(key.startsWith('action:'))delete map[key.slice(7)];stage3?.clearDraft?.(key);}
drafts=await installDrafts({modal,close,toast,ensureConsent,isConsented:()=>consent,onClear:resetInputs,onRestore:async(key,data)=>{if(key==='weekly'){answers=data.answers;question=data.question;scanStarted=true;location.hash='scan';await route();renderQuestion();}else if(key==='daily'){dailyValue=data.value;$('dailyNote').value=data.note;location.hash='home';daily();}else if(key==='diary'){diaryDraft=data.note;start('diary');}else if(key.startsWith('action:')){const id=key.slice(7);actionNotes[id]=data.note;actionInputs[id]={minutes:data.minutes??'',fit:data.fit??'not-rated'};location.hash='actions';await route();renderActions();}else{location.hash='measure';await route();stage3?.restoreDraft(key.split(':')[1],data);}}});
$('draftStatus').insertAdjacentHTML('afterend','<button class="btn" id="saveScanDraft">초안 보관 확인</button>');$('saveScanDraft').onclick=()=>ensureConsent(async()=>{if(await drafts.save('weekly',{answers:{...answers},question}))toast('이 기기에 초안을 보관했어요.');});
$('dailySaved').insertAdjacentHTML('afterend','<button class="btn" id="saveDailyDraft">완료 전 초안 보관</button>');$('saveDailyDraft').onclick=()=>ensureConsent(async()=>{if(await drafts.save('daily',{value:dailyValue,note:$('dailyNote').value}))toast('이 기기에 초안을 보관했어요.');});
daily();await route().catch(routeFailed);
