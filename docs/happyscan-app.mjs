...
const hasDraft=()=>Boolean(corrections?.hasDraft())||Object.values(actionNotes).some(v=>v.trim())||Object.keys(answers).length>0||dailyValue!==null||Boolean($('dailyNote').value.trim())||Boolean(diaryDraft.trim())||Boolean(stage2?.hasDraft())||Boolean(stage3?.hasDraft());
const actionNotes={};
...
async function route(){
  ...
  if(!contentLibrary&&page==='library'){({installContentLibrary}=await import('./happyscan-library.mjs'));contentLibrary=installContentLibrary({modal,close});contentLibrary.render();}
...
function renderActions(){
  ...
  $('myActions').innerHTML=chosen.length?`<div class="grid-3">${chosen.map(r=>{const a=ACTIONS.find(a=>a.id===r.actionId);return `<article class="card"><span class="badge">내가 선택한 실천</span><h3 style="margin-top:12px">${a.title}</h3><p class="small muted">${a.steps}</p><label for="note-${a.id}">실천하며 남기는 한 줄 · 선택</label><textarea id="note-${a.id}" data-action-note="${a.id}" maxlength="2000" rows="3">${esc(actionNotes[a.id]??'')}</textarea><div class="card-actions"><button class="btn primary" data-action-status="done" data-id="${a.id}">실천했어요</button><button class="btn" data-action-status="skipped" data-id="${a.id}">이번엔 쉬기</button></div></article>`;}).join('')}</div>`:'<div class="empty">아래에서 나에게 맞는 실천 하나를 골라보세요.<br>고르는 것만으로 완료 기록이 생기지는 않아요.</div>';
}
function report(){
...
document.addEventListener('input',e=>{if(e.target.dataset.actionNote)actionNotes[e.target.dataset.actionNote]=e.target.value;});
document.addEventListener('click',e=>{
  ...
  if(b.dataset.actionStatus)ensureConsent(async()=>{if(await save('action',{actionId:b.dataset.id,status:b.dataset.actionStatus,note:$('note-'+b.dataset.id)?.value.trim()||''},b)){delete actionNotes[b.dataset.id];toast(b.dataset.actionStatus==='done'?'실천을 남겼어요. 기분 변화는 별도로 기록해 보세요.':'쉬기로 한 선택을 기록했어요. 벌점은 없어요.');}});
