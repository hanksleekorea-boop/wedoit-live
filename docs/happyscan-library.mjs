...
export function installContentLibrary({modal,close,getRecords=()=>[]}={}){
  let installed=false;
  let recent=[];try{const saved=JSON.parse(sessionStorage.getItem('happyscan-library-view-v1')||'null');if(saved?.recent)recent=saved.recent.filter(id=>ACTIONS.some(a=>a.id===id)||[...PROGRAMS,...FOUR_WEEK_PROGRAMS].some(p=>p.id===id)).slice(0,5);}catch{}
  const remember=()=>{try{sessionStorage.setItem('happyscan-library-view-v1',JSON.stringify({query:$('librarySearch').value,kind:$('libraryKind').value,onlyAllowed:$('libraryAllowed').checked,recent}));}catch{}};
  const filter=()=>{const query=$('librarySearch').value.trim().toLocaleLowerCase(),kind=$('libraryKind').value,allowed=new Set(filteredActions(ACTIONS,getRecords().find(r=>r.type==='preferences')??{}).map(a=>a.id));let count=0;for(const id of ['contentCompass','contentActions','contentPrograms','contentHelp']){const section=$(id).closest('section');section.hidden=kind!=='all'&&kind!==id;for(const card of $(id).children){const excluded=card.dataset.libraryAction&&!allowed.has(card.dataset.libraryAction);card.hidden=Boolean(query)&&!card.textContent.toLocaleLowerCase().includes(query)||Boolean(excluded&&$('libraryAllowed').checked);if(card.querySelector('[data-exclusion-note]'))card.querySelector('[data-exclusion-note]').hidden=!excluded;if(!section.hidden&&!card.hidden)count++;}}$('libraryActionList').open=Boolean(query)||kind==='contentActions';$('libraryResults').textContent=count?`${count}개 항목을 찾았어요. 실천 목록은 펼쳐서 볼 수 있어요.`:'검색 결과가 없어요. 다른 말로 찾거나 조건을 지워주세요.';$('recentContent').innerHTML=recent.map(id=>{const a=ACTIONS.find(a=>a.id===id),p=[...PROGRAMS,...FOUR_WEEK_PROGRAMS].find(p=>p.id===id);if(a&&!allowed.has(a.id)&&$('libraryAllowed').checked)return '';return `<button class="btn" ${a?'data-action-detail':'data-program-preview'}="${id}">${escape((a||p).title)}</button>`;}).join('')||'<p>이 탭에서 최근에 본 콘텐츠가 없어요.</p>';remember();};
  const render=()=>{
    ...
    $('libraryResults').insertAdjacentHTML('beforebegin','<label><input type="checkbox" id="libraryAllowed" checked> 내 실천 조건에 맞는 내용만</label>');$('view-library').querySelector('.library-tools').insertAdjacentHTML('afterend','<details><summary>이 탭에서 최근 본 콘텐츠</summary><div class="row" id="recentContent"></div></details>');
    try{const state=JSON.parse(sessionStorage.getItem('happyscan-library-view-v1')||'null');if(state){$('librarySearch').value=typeof state.query==='string'?state.query.slice(0,200):'';if([...$('libraryKind').options].some(o=>o.value===state.kind))$('libraryKind').value=state.kind;if(typeof state.onlyAllowed==='boolean')$('libraryAllowed').checked=state.onlyAllowed;}}catch{}
    ...
    $('contentActions').innerHTML=ACTIONS.map(a=>`<article class="guide-card" data-library-action="${a.id}"><h3>${escape(a.title)}</h3><p>${escape(a.time)} · ${escape(a.steps)}</p><p data-exclusion-note class="notice" hidden>현재 개인 조건에서 제외된 참고 자료입니다. 실천 추천이 아니며 선택하려면 내 공간의 조건부터 확인하세요.</p><button class="btn" data-action-detail="${a.id}">방법과 주의사항 보기</button></article>`).join('');
    $('librarySearch').oninput=filter;$('libraryKind').onchange=$('libraryAllowed').onchange=filter;$('clearLibrary').onclick=()=>{$('librarySearch').value='';$('libraryKind').value='all';filter();};
  ...
  $('view-library').addEventListener('click',e=>{const b=e.target.closest('[data-action-detail],[data-program-preview]');if(!b)return;const id=b.dataset.actionDetail||b.dataset.programPreview;recent=[id,...recent.filter(v=>v!==id)].slice(0,5);remember();});
